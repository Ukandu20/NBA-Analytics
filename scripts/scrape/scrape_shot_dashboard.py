...
=============================
Fetch Teams ➤ Shot Dashboard stats (General, ShotClock, Dribbles, TouchTime,
ClosestDefender, ClosestDefender10ftPlus) for one or more NBA seasons,
season-types, per-modes (Totals, PerGame), and sub-ranges (for General),
...
Usage:
    python scrape_team_shot_dashboard.py \
        ...
        [--output-root data/raw/team_stats/shot_dashboard] \
        [--skip-existing] \
        [--delay 0.6]
"""
import argparse
...
PER_MODES    = ["Totals", "PerGame"]  # only supported modes
...
def fetch_shot_dashboard(
    ...
    # set sub-range when category == general
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape Teams ➤ Shot Dashboard with season-first layout."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=PER_MODES,
        help="Per-mode options: Totals, PerGame"
    )
    parser.add_argument(
        "--category", nargs="+", choices=CAT_MAP.keys(),
        default=list(CAT_MAP.keys()),
        help="Shot Dashboard categories to fetch"
    )
    parser.add_argument(
        "--output-root", type=Path,
        default=Path("data/raw/team_stats/shot_dashboard"),
            ...
            for season_type in args.season_type:
                for cat in args.category:
                    ...
                    for rng in ranges:
                        ...
                        fname = f"{season_type.lower().replace(' ', '_')}_{cat}{suffix}.csv"
                        fpath = out_dir / fname
                        ...
                        try:
                            df = fetch_shot_dashboard(
                                session, season, season_type, cat, per_mode, rng
                            )
                            if df.empty:
                                logging.warning(
                                    "No data: %s | %s | %s | %s",
                                    season, per_mode, season_type, cat
                                )
                                continue
                            ...
                            logging.error(
                                "Error %s | %s | %s | %s: %s",
                                season, per_mode, season_type, cat, e
                            )
