...
def scrape_nba_players():
    ...
    try:
        ...
        # Click the visible toggle switch instead of the checkbox
    toggle_label = wait.until(EC.element_to_be_clickable((By.XPATH, "//span[@aria-label='Show Historic Toggle']")))
    toggle_label.click()
    time.sleep(5)  # wait for the page to refresh with historic players
    ...
    if data and headers:
        ...
        df.to_csv("data/raw/all_players_raw.csv", index=False)
        print("✅ Player data scraped and saved to data/raw/all_players_raw.csv")
    else:
