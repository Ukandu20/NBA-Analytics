...
================================
Fetch Teams ➤ Opponent Shooting dashboard stats (General, ShotClock, Dribbles,
TouchTime, ClosestDefender, ClosestDefender10ftPlus) for one or more NBA seasons,
season-types, per-modes, and sub-ranges (for General), organizing output as:

    <output-root>/<season>/<perMode>/<season_type>/_<cat>[_<range>].csv

By default, fetches all PER_MODES (Totals, PerGame), all categories,
and all GENERAL_RANGE when applicable.

Usage:
    python scrape_team_opponent_shooting.py \
        ...
        [--output-root data/raw/team_stats/opponent_shooting] \
        [--skip-existing] \
        [--delay 0.6]
"""
import argparse
...
def fetch_opponent_shooting(
    ...
    params = {
        ...
        "LeagueID":       "00",
        "TeamID":         "0",
        # range filters blank by default
        "GeneralRange":       range_key and GENERAL_RANGE.get(range_key, "") or "",
        # other filters blank
        **{k: "" for k in ("LastNGames","Location","Outcome","Month","OpponentTeamID","Period","Conference","Division","VsConference","VsDivision","GameSegment","DateFrom","DateTo","PORound","PaceAdjust","PlusMinus","Rank",)}
    }
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Teams ➤ Opponent Shooting season-first layout.")
    ...
    parser.add_argument("--output-root", type=Path, default=Path("data/raw/team_stats/opponent_shooting"))
                ...
                for cat in args.category:
                    ...
                    for rng in ranges:
                        ...
                        if args.skip_existing and fpath.exists():
                            ...
                            continue
                        try:
                            df = fetch_opponent_shooting(session, season, season_type, cat, per_mode, rng)
                            if df.empty:
