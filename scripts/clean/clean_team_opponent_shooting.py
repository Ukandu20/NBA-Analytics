#!/usr/bin/env python3
"""
clean_team_opponent_shooting.py
===============================

Clean the raw CSVs downloaded by `scrape_team_opponent_shooting.py`.

Expected raw layout (per season)
--------------------------------
data/raw/team_stats/opponent_shooting/<season>/
└─ totals/
   ├─ regular_season_overall.csv
   ├─ playoffs_mid_range.csv
   ⋮
└─ per_game/
   ├─ regular_season_overall.csv
   ⋮
...
-------------------------------------------------------
data/processed/team_stats/opponent_shooting/<season>/
└─ totals/
...
3. Parse `season` (YYYY-YY) into     `season_start`, `season_end`
4. Convert all remaining columns to numeric (`errors="coerce"`)
   using utils.numeric_helpers.coerce_all_numeric
5. Drop exact duplicate rows
6. Preserve the original filenames & folder hierarchy

*No per-team split is created.*
...
RAW_ROOT  = ROOT / "data/raw/team_stats/opponent_shooting"
PROC_ROOT = ROOT / "data/processed/team_stats/opponent_shooting"
...
def parse_cli() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        ...
        description="Clean NBA team opponent-shooting tables.",
    )
