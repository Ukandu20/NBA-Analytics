#!/usr/bin/env python3
"""
clean_team_playtype.py
======================

Clean the raw CSVs saved by `scrape_team_playtype.py`
(teams ➤ Synergy Playtype dashboard).

Raw layout (per season)
-----------------------
data/raw/team_stats/playtype/<season>/
└─ totals/
   ├─ regular_season_isolation_offensive.csv
   ├─ playoffs_transition_defensive.csv
   ⋮
└─ per_game/
   ├─ regular_season_isolation_offensive.csv
   ⋮

Output layout (same shape, but under *processed*)
-------------------------------------------------
data/processed/team_stats/playtype/<season>/
└─ totals/
   ├─ regular_season_isolation_offensive.csv
   ⋮
└─ per_game/
...
4. Convert every remaining column to numeric (`errors="coerce"`)
   via `utils.numeric_helpers.coerce_all_numeric`
...
RAW_ROOT  = ROOT / "data/raw/team_stats/playtype"
PROC_ROOT = ROOT / "data/processed/team_stats/playtype"
...
def _add_season_bounds(df: pd.DataFrame) -> None:
    """Add numeric season_start / season_end from 'season' string."""
...
def clean_one_csv(src: pathlib.Path, dst: pathlib.Path, *, force: bool) -> None:
    ...
    # 2) fix team abbreviation
    _standardise_team_abbrev(df)

    # 3) add season bounds
    _add_season_bounds(df)

    # 4) numeric coercion (skip text + datetime)
...
def parse_cli() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        ...
        description="Clean NBA team Synergy playtype tables.",
    )
